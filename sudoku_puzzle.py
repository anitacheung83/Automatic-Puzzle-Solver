...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        True
        """
        for i, j in ((a, b) for a in range(self._n) for b in range(self._n)):
            if self._grid[i][j] == " ":
                new_lst = self.fail_fast_helper(i, j)
                if new_lst == []:
                    return True
        return False

    def fail_fast_helper(self, i: int, j: int) -> List:
        """ A fail_fast helper function that return a new_lst based on i and j.
        """
        lst = []
        for item in self._symbol_set:
            lst.append(item)
        new_lst = lst[:]
        for symbol in lst:
            if symbol in new_lst and symbol in self._row_set(i):
                new_lst.remove(symbol)
            if symbol in new_lst and symbol in self._column_set(j):
                new_lst.remove(symbol)
            if symbol in new_lst and symbol in self._subsquare_set(i, j):
                new_lst.remove(symbol)
    ...
    def has_unique_solution(self) -> bool:
        ...
        """
        solver = BfsSolver()
        actual = solver.solve(self)
        if actual == []:
            return False
        seen = actual[-1]
        solver1 = BfsSolver()
        expected = solver1.solve(self, {seen.__str__()})
